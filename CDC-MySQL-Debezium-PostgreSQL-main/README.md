...
docker run --tty --network cdc-mysql-debezium-postgresql-main_default  confluentinc/cp-kafkacat kafkacat -b kafka:9092 -L
  

docker run --rm --network cdc-mysql-debezium-postgresql-main_default ^
  confluentinc/cp-kafkacat ^
  kafkacat -b kafka:9092 -C -t customers -o beginning -f "\nKey (%K bytes): %k\t\nValue (%S bytes): %s\n\nPartition: %p\tOffset: %o\n--\n"


  https://github.com/AmanLonare/CDC-MySQL-Debezium-PostgreSQL
